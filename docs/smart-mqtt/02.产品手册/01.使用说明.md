...
本着对用户负责的态度，我们不提供这种比较结论。下面列举了个人认为比较不错的国产化产品可供各位参考：
| 项目   | 语言     | 出品方 | 地址                                                           |
|------|--------|-----|--------------------------------------------------------------|
...
而出于自荐的目的，smart-mqtt 有以下几类特色值得考虑：

- **开放性**。基于事件总线的架构模式，用户可将非 mqtt 的南向协议适配融入 smart-mqtt；也可将 mqtt 消息桥接至任意的北向服务。
- **亲和性**。在当下以 Java 为主流开发语言的环境下，smart-mqtt 天然适合绝大多数从业者。
- **可替代性**。smart-mqtt 完全遵循 MQTT 协议规范，用户无需担心被本产品捆绑。任何时候若觉得这款产品不再适合自己，可快速切换至其他同类产品。
- **纯国产血统**。符合信创场景需求：
